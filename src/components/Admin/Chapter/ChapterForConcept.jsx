...
const ChapterForConcept = ({
    ...
    return (
      <SCHOOLSEM.SemsterOptionContainer>
        {grades[school].map((opt, index) => (
          <SCHOOLSEM.DetailBtn key={index} $isSelected={grade === opt} value={opt} onClick={() => changeGrade(opt)}>
            {opt}
          </SCHOOLSEM.DetailBtn>
        ))}
  ...
  useEffect(() => {
    ...
    setCurrentChapter({ id: null, description: null, school: school, semester: sem, subject: sub });
