...
const NormalChapterForMock = ({ chapToAdd, setChapToAdd, mockGrade, mockSemester, setSelectingChapter }) => {
  const [school, setSchool] = useState("고");
  const [grade, setGrade] = useState(mockGrade === "고1" ? 1 : mockGrade);
  ...
  const onChapterClick = (thisChap) => {
    ...
    //   // subject: sub,
    // });
    setChapToAdd({ id: thisChap.id, description: thisChap.description });
        ...
        <CHAPTERSCOPE.ExpansionSection $isExpanded={thisChap.expansion} $isHovering={hoveredChapter === thisChap.id}>
          {level > 2 && thisChap.children.length === 0 ? (
            <CHAPTERSCOPE.CheckBox
              $isChecked={chapToAdd.id === thisChap.id}
              $isHovering={hoveredChapter === thisChap.id}
            >
              {chapToAdd.id === thisChap.id ? <FontAwesomeIcon icon={faCheck} /> : ""}
            </CHAPTERSCOPE.CheckBox>
          ) : (
...
export default NormalChapterForMock;
...
const CHAPMODAL = {
  Modal: styled.div`
    ...
    box-shadow: 0rem 0.5rem 3rem rgba(0, 0, 0, 0.5);
    width: 47.5rem;
    height: 65rem;
    overflow: hidden;
  `,
