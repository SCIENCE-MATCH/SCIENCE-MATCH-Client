...
const ManageQuestionPaper = () => {
  ...
  const getPapers = async () => {
    ...
    try {
      ...
      const url = "https://www.science-match.p-e.kr/teacher/question-paper";
  ...
  const deletePaper = async (paperId) => {
    try {
      const accessToken = getCookie("aToken");
      const url = "https://www.science-match.p-e.kr/teacher/question-paper/delete";

      const response = await Axios.post(url, [paperId], {
        headers: {
  ...
  const [selectedPaparIds, setSelectedPapaerIds] = useState([]);
  const selectPaper = (id) => {
    let tempPapers = [...receivedPapers];
    tempPapers.map((paper) => {
      if (paper.id === id) paper.selected = !paper.selected;
    });
    setReceivedPapers(tempPapers);

    let tempArr = [...selectedPaparIds];
  ...
  const handleBatchDownload = () => {
    receivedPapers.map((paper) => {
      if (selectedPaparIds.includes(paper.id)) {
        const link = document.createElement("a");
        link.href = paper.pdf;
        link.download = `${paper.title}.pdf`; // 다운로드될 파일 이름 설정
    ...
    <MQP.Wrapper>
      ...
      {isWarningBatch && (
        <WarningModal
          ...
          warningText={`선택한 문제지 ${selectedPaparIds.length}개를 모두 삭제합니다.`}
        />
      )}
      {isPresenting && <PresentPaper closeModal={closePresent} paper={previewPaper} paperIds={[presentId]} />}
      {isPresentingMany && <PresentPaper closeModal={closePresentMany} paperIds={selectedPaparIds} />}
      ...
      <MQP.ManageSection>
        ...
        <MQP.ListHeader>
          {attributes.map((att, index) => (
            <MQP.AttributeBox $width={widths[index]} $isTitle={att === "학습지명"}>
              {att}
            </MQP.AttributeBox>
          ))}
        </MQP.ListHeader>
        <MQP.ListContainer $shorten={selectedPaparIds.length > 0}>
          {papersToRender.map((paper, index) => (
            <MQP.QuestionPaperLine key={index}>
              ...
              <MQP.CellBox $width={widths[5]}>{paper.makerName} 선생님</MQP.CellBox>
        ...
        {selectedPaparIds.length > 0 ? (
          <MQP.BottomLine>
            <MQP.BottomText>{`질문 ${selectedPaparIds.length}개 선택됨`}</MQP.BottomText>
            <MQP.BatchActionBtn onClick={openPresentMany}>
              <FontAwesomeIcon icon={faUsers} style={{ marginBottom: "0.75rem" }} />
              일괄출제
            </MQP.BatchActionBtn>
            <MQP.BatchActionBtn onClick={downloadAll}>
