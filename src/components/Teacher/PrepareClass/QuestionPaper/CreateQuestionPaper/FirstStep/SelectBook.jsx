...
const SelectBook = ({
  ...
  const onQuestionClick = (question) => {
    ...
    if (selectedQuestions.some((q) => q.questionId === questionId)) {
      ...
      setSelectedQuestions([
        ...selectedQuestions,
        { ...question, imageURL: question.questionImg, description: question.chapterDescription, page: currentPage },
      ]);
    }
  };
  /** 1.1 => `1.(1)번` */
          ...
          <SELECTBOOK.ListContainer>
            {booksToRender.length === 0 ? (
              <SELECTBOOK.QuestionPaperLine>
                <SELECTBOOK.CellBox $width={22}>검색 결과가 없습니다.</SELECTBOOK.CellBox>
              </SELECTBOOK.QuestionPaperLine>
            ) : (
              booksToRender.map((book, index) => (
                <SELECTBOOK.QuestionPaperLine key={index}>
                  <SELECTBOOK.CellBox $width={widths[0]}>
                    {getKoreanGrade(book.school, book.semester)}
                  </SELECTBOOK.CellBox>
                  <SELECTBOOK.CellBox
                    $width={widths[1]}
                    onClick={async () => {
