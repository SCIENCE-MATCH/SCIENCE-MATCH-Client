...
const DetailOfMock = ({
  ...
  setSimpleChapters,
  mockList,
  mockChapters, //단원으로 선택시 필요
  handleGetQuestionOfMock,
}) => {
  const [scores, setScores] = useState([2, 3]);
  return (
    <CQP.QuestionSettings>
      <CQP.LabelContainer>
        <CQP.SettingLabel>모의고사 문제 구성</CQP.SettingLabel>
      </CQP.LabelContainer>
      <CQP.LabelContainer>
        <CQP.SettingLabel>배점</CQP.SettingLabel>
      </CQP.LabelContainer>
      <CQP.SettingLine>
        {["전체", 2, 3].map((opt) => (
          <CQP.DetailBtn
            $isSelected={opt === "전체" ? scores.length === 2 : scores.includes(opt) && scores.length === 1}
            onClick={() => (isNaN(opt) ? setScores([2, 3]) : setScores([opt]))}
            key={opt}
          >
            {opt}
            {isNaN(opt) || "점"}
      ...
      <QT.SummaryBox>
        학습지 문제 수 <QT.QuesNumber>{selectedQuestions.length}</QT.QuesNumber> 개
      </QT.SummaryBox>
      <QT.NextStepBtn
        onClick={() => {
          handleGetQuestionOfMock(mockList, scores);
          setSimpleChapters([]);
